...
const Signup = () => {
  ...
  const onSuccess = () => {
    setAlert({status:true,message:`${Credential.name} account created...`});
    console.log(alert);
    setTimeout(()=>{
  ...
  const Submit_SingUp_Form = async (e) => {
    ...
    if (json.success) {
      ...
      await(setUsername(Credential.name))
      console.log('did the name change to =',Credential.name,username)
